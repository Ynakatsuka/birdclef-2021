...
def schedule(config, epoch):
  t = epoch / config.trainer.swa.start
  lr_ratio = config.trainer.swa.lr / config.trainer.optimizer.params.lr
  if t <= 0.5:
    factor = 1.0
  elif t <= 0.9:
    ...
    factor = lr_ratio
  return config.trainer.optimizer.params.lr * factor
...
def get_checkpoints(config):
    ...
    num_checkpoint = config.trainer.swa.num_checkpoint

    if 'epoch_end' in config.trainer.swa:
        epoch_end = config.trainer.swa.epoch_end
    else:
        epoch_end = None

    checkpoint_dir = os.path.join(config.trainer.train.dir, 'checkpoint')
...
def run(config):
    ...
    # build datasets
    config.trainer.dataset.splits = [v for v in config.trainer.dataset.splits if v.split == 'train' or v.split == 'train_all']
    print(config.trainer.dataset.splits)
    ...
    for i, checkpoint in enumerate(checkpoints[1:]):
        ...
        if 'ema' in config.trainer.swa:
            moving_average(model, model2, config.trainer.swa.ema)
        else:
